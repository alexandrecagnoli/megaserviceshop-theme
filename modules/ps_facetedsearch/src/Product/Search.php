...
class Search
{
    ...
    private function addControllerSpecificFilters()
    {
        ...
        // La condition « un filtre id_category a deja ete pose par le client »
        // etait suivie d'un `return` qui sortait de TOUTE la methode, et sautait
        // donc le Hook::exec('actionFacetedSearchFilters') qui la termine. Des
        // qu'une facette Categories etait cochee, les filtres des modules tiers
        // disparaissaient : chez nous la compatibilite moto, d'ou des pieces
        // incompatibles listees comme compatibles. On ne saute plus que le bloc
        // concerne, en fusionnant la condition dans le `if` englobant.
        if ($this->query->getQueryType() == 'category'
            && empty($this->getSearchAdapter()->getFilter('id_category'))) {
