...
class SearchProvider implements FacetsRendererInterface, ProductSearchProviderInterface
{
    ...
    public function runQuery(
        ...
        // PATCH Megaservice (ps_facetedsearch 4.0.2) — cf. TECH_DEBT.md
        //
        // generateCacheKeyForQuery() ne connait que la boutique, la devise, la
        // langue, le pays, le controleur et les facettes cochees. Si un module
        // tiers a restreint la population via actionFacetedSearchFilters (chez
        // nous : la compatibilite avec la moto du garage), deux pages dont la
        // population differe partagent le meme hash. Les compteurs affiches
        // sont alors ceux de la page non filtree : « Bagagerie (100) » menant
        // a « Aucun produit ».
        //
        // Ajouter le discriminant au hash multiplierait les entrees par le
        // nombre de motos. On se contente donc de ne pas utiliser le cache
        // quand la population a ete restreinte : seules ces pages recalculent.
        $initialPopulation = $facetedSearch->getSearchAdapter()->getInitialPopulation();
        $populationRestrictedByModule = $initialPopulation !== null
            && !empty($initialPopulation->getFilter('id_product'));

        $useBlockCache = !$populationRestrictedByModule
            && $this->module->shouldCacheController($query->getQueryType());
